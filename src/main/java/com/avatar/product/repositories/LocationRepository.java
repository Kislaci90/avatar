...
public interface LocationRepository extends
    ...
    @Query("SELECT l FROM Location l LEFT JOIN FETCH l.fields fields WHERE l.id = :id")
    Location findLocationById(@Param("id") Long id);

    @NotNull
    @Query("SELECT l FROM Location l LEFT JOIN FETCH l.fields fields")
    Page<Location> findAll(@NotNull Pageable pageable);

    @NotNull
    @Query("SELECT l FROM Location l LEFT JOIN FETCH l.fields fields WHERE l.name ILIKE %:name%")
